...
else{key=key.replace(/]]>/g,']]]]><![CDATA[>');value=value.replace(/]]>/g,']]]]><![CDATA[>');}
...
readyStateChange:function(r){try{if(r.status==0||r.status==200){if(xajaxConfig.debug)xajax.DebugMessage("Received:\n"+r.responseText);if(r.responseXML&&r.responseXML.documentElement&&r.responseXML.documentElement.nodeName=='xjx')
this.processResponse(r.responseXML);else if(this.isEventFunction(this.callOptions.responseHandler)){this.callOptions.responseHandler(r);this.completeResponse();}
...
else if(cmd=="rp"){cmdFullname="replace";search=data['s'];data=data['r'];this.replace(id,property,search,data)
}
